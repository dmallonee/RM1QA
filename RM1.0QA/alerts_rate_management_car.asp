...
<% Response.Expires = -1
   ...
   'response.write "Error test - 2 <br>"
	...
	adoCmd9.Parameters.Append adoCmd9.CreateParameter("@org_id",              3, 1, 0, Null)
	adoCmd9.Parameters.Append adoCmd9.CreateParameter("@schedule_type_id",    3, 1, 0, 3)
	adoCmd9.Parameters.Append adoCmd9.CreateParameter("@user_id",             3, 1, 0, strUserId)
		
	Set adoRS13 = adoCmd9.Execute
	Set adoCmd9 = Nothing
	...
	If intRuleId > 0 Then
        ...
        intMinMaxId = adoRS5.Fields("minmax_schedule_id").Value
        intThresholdId = adoRS5.Fields("threshold_schedule_id").Value
        intTpId = adoRS5.Fields("tp_minmax_schedule_id").Value
    ...
    <tr>
      ...
      <table width="1108" border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" background="images/alt_color.gif">
        <tr valign="bottom">
          ...
          To search for a rule, enter a login id, or a portion of the 
...
<td width="125" height="25" bgcolor="#FFFFFF" bordercolor="#555566" bordercolorlight="#999999" bordercolordark="#777777" style="border: 1px solid #40618F; padding: 2px">
    ...
    <tr>
      ...
      <select size="4" name="city_cd" style="width:200; font-family:Verdana; font-size:10pt" multiple >
     	 <% intLoopCount = 0                                     %>
         <% strCityArr = Split(strCityCd,",") %>
         <% While (adoRS6.EOF = False) And (intLoopCount < 800)  %>
         <%     opselected = 0
                for i=0 to UBound(strCityArr)
                    If Trim(strCityArr(i)) = Trim(adoRS6.Fields("city_cd").Value) Then
                        opselected = 1
                    end if
                next
                If (opselected = 0) And (strCityCd <> "") Then %>
...
-->      
    ...
    <tr>
      ...
      <%if Session("minmax")="True" Then %> 
      Min/Max Schedule</td>      <td width="510" height="22" colspan="8">
      
		<select name="minmax_schedule_id" style="width:300; font-family:Verdana; font-size:10pt; height:24" size="1">
			<% If intMinMaxId = 0 Then %>
     		<option selected value="0">(No Min/Max Schedule)</option>
     		<% Else %>
     		<option value="0">(No Min/Max Schedule)</option>
			<% End If %>
			<% intLoopCount = 0                                      %>
     		<% While (adoRS11.EOF = False) And (intLoopCount < 200)  %>
				<% If intMinMaxId = adoRS11.Fields("car_rate_rule_schedule_id").Value Then %>
    ...
    <tr>
      ...
      &nbsp;&nbsp;&nbsp; Threshold Schedule:</td>      <td width="510" height="22" colspan="8">
		...
		<select name="threshold_schedule_id" style="width:300; font-family:Verdana; font-size:10pt; height:24" size="1">
			<% If intThresholdId = 0 Then %>
     		<option selected value="0">(No Threshold Schedule)</option>
     		<% Else %>
     		<option value="0">(No Threshold Schedule)</option>
			<% End If %>
			<% intLoopCount = 0                                      %>
     		<% While (adoRS12.EOF = False) And (intLoopCount < 200)  %>
				<% If intThresholdId = adoRS12.Fields("car_rate_rule_schedule_id").Value Then %>
    ...
    <tr>
      ...
      <td width="217" height="22">
          &nbsp;</td>
      <td width="210" height="22">
      &nbsp;&nbsp;&nbsp; TP Min/Max Schedule:</td>      <td width="510" height="22" colspan="8">
      
		<select name="tp_schedule_id" style="width:300; font-family:Verdana; font-size:10pt; height:24" size="1">
			<% If intTpId = 0 Then %>
     		<option selected value="0">(No TP Min/Max Schedule)</option>
     		<% Else %>
     		<option value="0">(No TP Min/Max Schedule)</option>
			<% End If %>
			<% intLoopCount = 0                                      %>
     		<% While (adoRS13.EOF = False) And (intLoopCount < 200)  %>
				<% If intTPId = adoRS13.Fields("car_rate_rule_schedule_id").Value Then %>
		   		<option selected value="<%=adoRS13.Fields("car_rate_rule_schedule_id").Value %>"><%=adoRS13.Fields("car_rate_rule_schedule_desc").Value  %></option>
	     		<% Else %>
		   		<option value="<%=adoRS13.Fields("car_rate_rule_schedule_id").Value %>"><%=adoRS13.Fields("car_rate_rule_schedule_desc").Value  %></option>
				<% End If %>
	   			<% adoRS13.MoveNext
	   			   intLoopCount = intLoopCount + 1
	   		    %>
        ...
        <a title="Click to edit or create a TP Min/Max schedule" href="tp_maxmin_schedule_a.aspx" onclick="window.open('tp_maxmin_schedule_a.aspx?user_id=<%=strUserId %>','manage_schedule','toolbar=no,status=no,scrollbars=yes,resizable=no,width=820,height=600'); return false;">Manage&nbsp;TP&nbsp;Schedules</a></td>      <td width="262" height="22">&nbsp;</td>
    </tr>
    <%end if %>
    <tr>
      <td width="8" height="22">&nbsp;</td>
