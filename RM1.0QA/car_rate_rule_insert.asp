...
<% Response.Expires = -1
   ...
   Server.ScriptTimeout = 60 '180
	...
	Else
		...
		adoCmd.Parameters.Append adoCmd.CreateParameter("@city_cd", 200, 1, 511,  Request("city_cd"))
		...
		If Request("situation_cd") >= 500 And IsNumeric(Request("situation_amt")) Then
			adoCmd.Parameters.Append adoCmd.CreateParameter("@situation_cd", 3, 1, 0,  Request("situation_amt"))
		Else
		...
		If Request("response_cd") >= 500 And IsNumeric(Request("response_amt")) Then
			adoCmd.Parameters.Append adoCmd.CreateParameter("@response_cd", 3, 1, 0,  Request("response_amt"))
		Else
    	...
    	adoCmd.Parameters.Append adoCmd.CreateParameter("@range_max", 6, 1, 0, Request("minmax_schedule_id"))
		adoCmd.Parameters.Append adoCmd.CreateParameter("@range_min", 6, 1, 0, 99999)
        if Request("minmax_schedule_id") then
		    adoCmd.Parameters.Append adoCmd.CreateParameter("@minmax_schedule_id", 3, 1, 0, Request("minmax_schedule_id"))
        else
		    adoCmd.Parameters.Append adoCmd.CreateParameter("@minmax_schedule_id", 3, 1, 0, 0)
        end if
        if Request("threshold_schedule_id") then
		    adoCmd.Parameters.Append adoCmd.CreateParameter("@threshold_schedule_id", 3, 1, 0, Request("threshold_schedule_id"))
        else
		    adoCmd.Parameters.Append adoCmd.CreateParameter("@threshold_schedule_id", 3, 1, 0, 0)
        end if
        if Request("tp_schedule_id") then
		    adoCmd.Parameters.Append adoCmd.CreateParameter("@tp_minmax_schedule_id", 3, 1, 0, Request("tp_schedule_id"))
        else
		    adoCmd.Parameters.Append adoCmd.CreateParameter("@tp_minmax_schedule_id", 3, 1, 0, 0)
        end if
...
'		End If
	...
	adoCmd.Parameters.Append adoCmd.CreateParameter("@on_success_ids", 200, 1, 4096, Null)
	adoCmd.Parameters.Append adoCmd.CreateParameter("@on_failure_ids", 200, 1, 4096, Null)

	adoCmd.Parameters.Append adoCmd.CreateParameter("@additional_days", 17, 1, 0, Request.Form("additional_days"))
	
	If Request.Form("sandbox") = 1 Then
		adoCmd.Parameters.Append adoCmd.CreateParameter("@sandbox",        11, 1, 0, 1)
	Else
		adoCmd.Parameters.Append adoCmd.CreateParameter("@sandbox",        11, 1, 0, 0)
	End If

	If Request.Form("extraday") Then
		adoCmd.Parameters.Append adoCmd.CreateParameter("@xday_processing",        11, 1, 0, 1)
	Else
		adoCmd.Parameters.Append adoCmd.CreateParameter("@xday_processing",        11, 1, 0, 0)
	End If

	adoCmd.Parameters.Append adoCmd.CreateParameter("@custom_response",  200, 1, 255, Request.Form("custom_response"))
	adoCmd.Parameters.Append adoCmd.CreateParameter("@custom_situation", 200, 1, 255, Request.Form("custom_situation"))
	adoCmd.Parameters.Append adoCmd.CreateParameter("@additional_days_prior", 17, 1, 0, Request.Form("additional_days_prior"))
	
	If Request("set_to_self") = "True" Then
		adoCmd.Parameters.Append adoCmd.CreateParameter("@set_to_self", 11, 1, 0, 1)
	Else
		adoCmd.Parameters.Append adoCmd.CreateParameter("@set_to_self", 11, 1, 0, 0)
	End If
	...
	If Request("debug") <> "true" Then
		...
		If err.number = 0 Then
			If Request.Form("sandbox") = 1 Then
				Server.Transfer "alerts_rate_management_car_sandbox.asp"
			Else
				Server.Transfer "alerts_rate_management_car.asp"
			End If
