...
<%  Response.Expires = -1
	...
	bolOverrideOption  = True ' Hard-coded for now False
	...
	'	End If
		...
		Else
			adoCmd.Parameters.Append adoCmd.CreateParameter("@rate_cd"      , 200, 1, 50, Null)
		End If
...
<a target="_self" href="rate_change_report_override_filtered.asp?reportrequestid=<%=Request("reportrequestid") %>&security_code=<%=Escape(Request("security_code")) %>">Overrideable Version</a> |
<% End If %>
	</font></td>
    </tr>
   </table>
...
<form name="rate_change" method="POST" action="rate_change_request.asp">
  <table border="1" bordercolor="#FFFFFF" id="suggestions" width="<%=intTableWidth %>" cellspacing="0" cellpadding="0" >
    <tr>
      ...
      <th class="profile_header"><font size="2">Custom for ABG</font></th>
      <th class="profile_header"><font size="2">Custom for ABG</font></th>
      <th class="profile_header" height="45"><font size="2">Custom for ABG</font></th>
<% If bolShowUtilization = True Then %>
...
%>
    ...
    <%=FormatDateTime(adoRS.Fields("arv_dt").Value, 2) %>
    </td>
    <td class="<%=strClass %>_ctr" width="45">
    <font size="-1">
    <%=adoRS.Fields("lor").Value%></font>
    </td>
