...
<%  Response.Expires = -1
	...
	If IsNumeric(Request("reportrequestid")) Then
		...
		adoCmd.Parameters.Append adoCmd.CreateParameter("@shop_request_id", 3, 1, 0, Request.QueryString("reportrequestid"))
		adoCmd.Parameters.Append adoCmd.CreateParameter("@guid",           72, 1, 0, Unescape(Request.QueryString("security_code")))
...
confirmed.</font></p>
<p align="center">&nbsp;</p>
<form method="POST" action="rate_change_receipt.asp" name="rate_change_receipt">
  <p align="center"><input type="submit" value="View Report" name="view_report"></p>
  <input type="hidden" name="reportrequestid" value="<%=Request.QueryString("reportrequestid") %>">
  <input type="hidden" name="security_code" value="<%=Unescape(Request.QueryString("security_code")) %>">
</form>
